...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        return minact
        '''
        closetGhostPosit = None
        closetGhostDist = -10000000
        tempVar = 0
        bestAct = None

        for ghostPosDistr in livingGhostPositionDistributions:
            belKeys = ghostPosDistr.keys()
            
            for posKey in belKeys:
                if ghostPosDistr[posKey] > tempVar:
                    tempVar = ghostPosDistr[posKey]
                    Distancer = self.distancer
                    closetGhostDist = Distancer.getDistance(pacmanPosition,posKey)
                    closetGhostPosit = posKey

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            Distancer = self.distancer
            successorDistance = Distancer.getDistance(closetGhostPosit, successorPosition)
            if (successorDistance < closetGhostDist):
                bestAct = action
                closetGhostDist = successorDistance
